...
void CMisc::OnCreateMove( CUserCmd* pCmd )
{
	...
	ConVar* nosky = Interfaces::GetConVar()->FindVar("sv_skyname");

	if (Settings::Misc::misc_SkyName) { if (skybox) skybox->SetValue(Settings::Misc::misc_SkyName); }
	if (Settings::Misc::misc_NoSky) { if (nosky) nosky->SetValue("sky_l4d_rural02_ldr"); }

	if (Settings::Misc::misc_SVCheat)
	{
	
			ConVar* sv = Interfaces::GetConVar()->FindVar("sv_cheats");
	...
	if (Settings::Misc::misc_Snow)
	{
		...
		ConVar* sv = Interfaces::GetConVar()->FindVar("mat_drawgray");
