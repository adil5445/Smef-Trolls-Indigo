	...
	void OnRender()
	{
		if (g_pRender && !Interfaces::Engine()->IsTakingScreenshot() && Interfaces::Engine()->IsActiveApp())
		{
			...
			if (!Interfaces::Engine()->IsConnected())
				g_pRender->Text(3, 15, false, true, Color::White(), CHEAT_NAME);
			{
				if (g_pEsp)
					g_pEsp->OnRender();
			...
			if (Settings::Esp::esp_Time)
				g_pRender->Text(3, 3, false, true, Color::White(), std::asctime(std::localtime(&result)));
	...
	void OnRenderGUI()
	{
		...
		if (ImGui::Begin(CHEAT_NAME, &bIsGuiVisible, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize))
		{
			...
			else if (tabSelected == 6) // Misc
			{
				...
				ImGui::PopItemWidth();
				ImGui::Separator();
				ImGui::Text("Warning: May Cause UT Bans (for OP Commands)");
				ImGui::Checkbox("SV Cheats Bypass", &Settings::Misc::misc_SVCheat);
				ImGui::Separator();
				ImGui::Text("This Project is a free open source project made possible by the coders of http://smef.pw/indigo");
			}
